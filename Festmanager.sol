...
contract FestManager {
    ...
    event EventCreated(uint eventId, string title);
    event Registered(uint eventId, address attendee);
    ...
    function vote(uint _eventId) public {
        ...
        emit Voted(_eventId, msg.sender); // Fixed typo: `voted` → `Voted`
    }
    ...
    function markPresent(uint _eventId, address _attendee) public onlyAdmin {
        ...
        require(!present[_eventId][_attendee], "Already marked");
